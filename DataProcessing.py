...
class DataProcessing:
    ...
    def processCSV(self, data_path):
        ...
        #     [vocab.update(i) for i in story.split()]


        storySentences = []
        for story in stories:
            storySentences += [i for i in story.split('۔')]

        labelSet = list(set(df['Labels']))
        count = []
        for label in labelSet:
            count.append(list(df['Labels']).count(label))

        tokenizer = Tokenizer(num_words=11, filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n')
        tokenizer.fit_on_texts(storySentences)

        longestStory = max(stories, key=len)
        longestLength = len(longestStory)

        shortestStory = min(stories, key=len)
        shortestLength = len(shortestStory)

        labelCountDict = {labelSet[i]: count[i] for i in range(len(count))}
        longShortStories = {"Longest Story": [longestStory, longestLength],
                            "Shortest Story": [shortestStory, shortestLength]}
        ...
        mostFrequentWords = {}
        for key, value in sorted(dict(tokenizer.word_counts).items(), key=lambda x: x[1], reverse=True):
            if key not in stopWords:
                mostFrequentWords[key] = value
            if len(mostFrequentWords.items())==10:
                break


        # mostFrequentWords = sorted(dict(tokenizer.word_counts).items(), key=lambda x: x[1], reverse=True)[:10]
        VocabSize = len(tokenizer.word_index)

        return VocabSize, mostFrequentWords, labelCountDict, longShortStories
